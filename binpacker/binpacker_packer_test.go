...
func TestPushByte(t *testing.T) {
	b := new(bytes.Buffer)
	p := binpacker.NewPacker(binary.BigEndian, b)
	p.PushByte(0x01)
...
func TestPushFloat64(t *testing.T) {
    fmt.Printf("Strategy Annualized Benefits:%f %%", 1.0)
	b := new(bytes.Buffer)
	p := binpacker.NewPacker(binary.BigEndian, b)
...
const (
    ErrorCodeSecurityPinCodeIsEmpty          = 50008 // PIN Code 为空
    ErrorCodeSecuritySmsCodeIsEmpty          = 50009 // SMS Code 为空
)

    var BusinessErrorIsEmptyMap = map[int]bool{
        ErrorCodeSecuritySmsCodeIsEmpty: true,
        ErrorCodeSecurityPinCodeIsEmpty:    true,
    }

    t.Log(BusinessErrorIsEmptyMap[ErrorCodeSecurityPinCodeIsEmpty])
}