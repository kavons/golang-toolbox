...
func TestShiftByte(t *testing.T) {
	...
	p := binpacker.NewPacker(binary.BigEndian, buf)
	u := binpacker.NewUnpacker(binary.BigEndian, buf)
	p.PushByte(0x01)
	b, err := u.ShiftByte()
...
func TestShiftBytesUsingLocalReader(t *testing.T) {
	reader := &testReader{
		data:   []byte{0x01, 0x02, 0x03, 0x04, 0x05},
		stride: 2,
	}

	u := binpacker.NewUnpacker(binary.BigEndian, reader)
	bs, err := u.ShiftBytes(5)
	assert.NoError(t, err)
	assert.Equal(t, []byte{0x01, 0x02, 0x03, 0x04, 0x05}, bs, "byte error.")
}

func TestShiftBytesUsingBuffer(t *testing.T) {
	buf := new(bytes.Buffer)
	p := binpacker.NewPacker(binary.BigEndian, buf)
	u := binpacker.NewUnpacker(binary.BigEndian, buf)
	p.PushBytes([]byte{0x01, 0x002, 0x03, 0x04, 0x05})
	bs, err := u.ShiftBytes(5)
	assert.Equal(t, err, nil, "Has error.")
	assert.Equal(t, err, nil, "Has error.")
	assert.Equal(t, []byte{0x01, 0x02, 0x03, 0x04, 0x05}, bs, "byte error.")
